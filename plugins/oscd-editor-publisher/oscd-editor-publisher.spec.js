...
import PublisherPlugin, { PUBLISHER_TYPE_LOCAL_STORAGE_KEY, } from './oscd-editor-publisher.js';
...
function publisherFixture() {
    return fixture(html `<oscd-editor-publisher
      .doc=${new DOMParser().parseFromString('<SCL></SCL>', 'application/xml')}
    ></oscd-editor-publisher>`);
}
function publisherTypeButton(plugin, label) {
    return plugin.shadowRoot.querySelector(`oscd-outlined-segmented-button[label="${label}"]`);
}
describe('Publisher plugin', () => {
    ...
    it('stores the selected publisher type in localStorage', async () => {
        ...
        dataSetButton.click();
        expect(localStorage.getItem(PUBLISHER_TYPE_LOCAL_STORAGE_KEY)).to.equal('DataSet');
    });
    it('keeps selected segmented button icons instead of checkmarks', async () => {
        const plugin = await publisherFixture();
        const buttons = plugin.shadowRoot.querySelectorAll('oscd-outlined-segmented-button');
        const gooseButton = publisherTypeButton(plugin, 'GOOSE');
        const materialButton = gooseButton.shadowRoot.querySelector('button');
        buttons.forEach((button) => {
            expect(button.noCheckmark).to.be.true;
        });
        expect(materialButton.classList.contains('md3-segmented-button--without-checkmark')).to.be.true;
    });
});
//# sourceMappingURL=oscd-editor-publisher.spec.js.map
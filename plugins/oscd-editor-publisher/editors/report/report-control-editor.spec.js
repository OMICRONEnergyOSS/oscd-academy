...
async function selectReport(editor, name) {
    const reportListItem = Array.from(editor.shadowRoot.querySelectorAll('oscd-list-item.report-list-item')).find(item => item.querySelector('[slot="headline"]')?.textContent?.trim() === name);
    reportListItem.click();
    await editor.updateComplete;
}
function appendClientAccessPointLn(doc) {
...
function clientLnDialog(editor) {
    return editor.shadowRoot.querySelector('clientln-assignment-dialog');
}
describe('ReportControl editor component', () => {
    let editor;
    let editEvent;
    beforeEach(async () => {
        const doc = new DOMParser().parseFromString(reportControlDoc, 'application/xml');
        editor = await fixture(html `<report-control-editor .doc="${doc}"></report-control-editor>`);
        editEvent = spy();
        window.addEventListener('oscd-edit-v2', editEvent);
    });
    it('allows to insert new ReportControl element', async () => {
        const createButton = editor.shadowRoot.querySelector('oscd-icon-button.create-report-control');
        createButton.click();
        ...
        expect(editEvent.args[0][0].detail.edit.node.tagName).to.equal('ReportControl');
    });
    it('allows to remove and existing ReportControl element', async () => {
        const deleteMenuItem = Array.from(editor.shadowRoot.querySelectorAll('oscd-menu-item')).find(item => item.querySelector('[slot="headline"]')?.textContent?.trim() ===
            'Delete');
        deleteMenuItem.click();
        expect(editEvent).to.have.been.calledOnce;
        expect(isRemove(editEvent.args[0][0].detail.edit[0])).to.be.true;
        expect(editEvent.args[0][0].detail.edit[0].node.tagName).to.equal('ReportControl');
    });
    it('allows to insert new DataSet and link with existing ReportControl', async () => {
        ...
        expect(editEvent.args[0][0].detail.edit[0].node.tagName).to.equal('DataSet');
    });
    it('allows to change an existing DataSet', async () => {
        await setViewport({ width: 800, height: 800 });
        await selectReport(editor, 'rp2');
        editor.selectDataSet(editor.doc.querySelector('DataSet[name="datSet2"]'));
        expect(editEvent).to.have.been.calledOnce;
        expect(isSetAttributes(editEvent.args[0][0].detail.edit)).to.be.true;
        expect(editEvent.args[0][0].detail.edit.element.tagName).to.equal('ReportControl');
        expect(editEvent.args[0][0].detail.edit.attributes.datSet).to.equal('datSet2');
    });
    it('clears selected ReportControl details when the document changes', async () => {
        ...
        expect(editor.shadowRoot.querySelector('report-control-element-editor')).to
            .exist;
        editor.doc = new DOMParser().parseFromString('<SCL></SCL>', 'application/xml');
        await editor.updateComplete;
        expect(editor.shadowRoot.querySelector('report-control-element-editor')).to
            .not.exist;
    });
    it('shows report context menu actions', () => {
        const actionButton = editor.shadowRoot.querySelector('oscd-icon-button.report-actions-button');
        const menuItems = Array.from(editor.shadowRoot.querySelectorAll('oscd-menu-item'));
        const labels = menuItems.map(item => item.querySelector('[slot="headline"]')?.textContent?.trim());
    ...
    it('does not render ClientLN candidates before opening the dialog', () => {
        ...
        expect(editor.shadowRoot.querySelector('oscd-tree')).to.not.exist;
    });
    it('builds legacy-style ClientLN candidates from all IEDs', async () => {
        const doc = new DOMParser().parseFromString(`<SCL>
        <Communication>
          <SubNetwork name="A">
    ...
    it('keeps sibling ClientLNs under the same LD in the tree', async () => {
        ...
        const logicalNodes = assignmentEditor
            .clientLnTreeData(assignmentEditor.clientLogicalNodesForReport(reportControl))
            .find(node => node.id === 'IED:PUB_A')
            ?.children?.find(node => node.id === 'IED:PUB_A|AP:AP1')
            ?.children?.find(node => node.id === 'IED:PUB_A|AP:AP1|LD:LD_A')
            ?.children?.map(node => node.id) ?? [];
        expect(logicalNodes).to.include('PUB_A|AP1|LD_A||TCTR|1');
        expect(logicalNodes).to.include('PUB_A|AP1|LD_A||XCBR|1');
    });
    it('assigns a ReportControl without RptEnabled to a ClientLN', () => {
        const reportControl = editor.doc.querySelector('ReportControl[name="rp1"]');
        const logicalNode = editor.doc.querySelector('IED[name="IED2"] AccessPoint[name="AP1"] > LN[lnClass="IHMI"]') ?? appendClientAccessPointLn(editor.doc);
        const assignmentEditor = clientLnDialog(editor);
        assignmentEditor.clientLnAssignmentReport = reportControl;
        ...
        expect(edits[1].node.getAttribute('prefix')).to.equal(logicalNode.getAttribute('prefix'));
    ...
    it('assigns a ReportControl with RptEnabled to a ClientLN', () => {
        const reportControl = editor.doc.querySelector('ReportControl[name="rp2"]');
    ...
    it('shows already assigned ClientLNs in the tree selection', async () => {
        ...
        assignmentEditor.initialClientLnIds = ['IED2|AP1|LD0|IHMI|IHMI|1'];
        assignmentEditor.selectedClientLnIds = ['IED2|AP1|LD0|IHMI|IHMI|1'];
        assignmentEditor.expandedClientLnNodeIds = [
            'IED:IED2',
            'IED:IED2|AP:AP1',
        ];
        await assignmentEditor.updateComplete;
        const tree = assignmentEditor.shadowRoot.querySelector('oscd-tree');
        expect(tree.selectedIds).to.deep.equal(['IED2|AP1|LD0|IHMI|IHMI|1']);
    });
    it('shows ClientLN selection count at max', async () => {
        ...
        appendClientAccessPointLn(editor.doc);
        await assignmentEditor.updateComplete;
        const count = assignmentEditor.shadowRoot.querySelector('.client-ln-count');
        expect(count?.textContent?.trim()).to.equal('4/4 clients');
    });
    it('labels the ClientLN selected-only toggle by current action', async () => {
        const reportControl = editor.doc.querySelector('ReportControl[name="rp2"]');
        const assignmentEditor = clientLnDialog(editor);
        assignmentEditor.clientLnAssignmentReport = reportControl;
        await assignmentEditor.updateComplete;
        const toggle = assignmentEditor.shadowRoot.querySelector('.show-selected-client-lns');
        expect(toggle.getAttribute('title')).to.equal('Show selected Client LNs');
        expect(toggle.getAttribute('aria-label')).to.equal('Show selected Client LNs');
        toggle.click();
        await assignmentEditor.updateComplete;
        expect(toggle.getAttribute('title')).to.equal('Show all Client LNs');
        expect(toggle.getAttribute('aria-label')).to.equal('Show all Client LNs');
    });
    it('keeps ClientLN tree selection within the max client count', async () => {
        ...
        const disabledRows = Array.from(tree.shadowRoot.querySelectorAll('[role="treeitem"][aria-disabled="true"]'));
        const newLogicalNodeName = `${newLogicalNode.getAttribute('prefix') ?? ''}${newLogicalNode.getAttribute('lnClass') ?? ''}${newLogicalNode.getAttribute('inst') ?? ''}`;
        expect(disabledRows.some(row => row.getAttribute('aria-label')?.includes(newLogicalNodeName))).to.be.true;
    });
});
//# sourceMappingURL=report-control-editor.spec.js.map
...
function subDataObjectsObject(sdo, sDiOrDai) {
    ...
    Array.from(doType.querySelectorAll(':scope > SDO, :scope > DA')).forEach((sDoOrDa) => {
        if (sDoOrDa.tagName === 'SDO') {
            ...
            const sdi = sDiOrDai?.querySelector(`:scope > SDI[name="${name}"]`) ?? undefined;
            ...
            const sdiOrDai = sDiOrDai?.querySelector(`:scope > SDI[name="${name}"], :scope > DAI[name="${name}"]`) ?? undefined;
            let desc = sdiOrDai?.getAttribute('desc');
...
function dataObjectObject(dO, dOI) {
    ...
    Array.from(doType.querySelectorAll(':scope > SDO, :scope > DA')).forEach((sDoOrDa) => {
        if (sDoOrDa.tagName === 'SDO') {
            const name = sDoOrDa.getAttribute('name') ?? 'UNKNOWN_SDO';
            const sDi = dOI?.querySelector(`:scope > SDI[name="${name}"]`) ?? undefined;
            ...
            const dAi = dOI?.querySelector(`:scope > DAI[name="${name}"]`) ?? undefined;
...
function lDeviceObject(lDevice) {
    ...
    Array.from(lDevice.querySelectorAll('LN0,LN')).forEach((anyLn) => {
        ...
        const anyLnClass = `${anyLn.getAttribute('prefix') ?? ''} ${anyLn.getAttribute('lnClass') ?? 'UNKNOWN_INST'} ${anyLn.getAttribute('inst') ?? ''}${desc ? ` (${desc})` : ''}`;
...
export function dataAttributeTree(server) {
    const tree = {};
    Array.from(server.querySelectorAll('LDevice')).forEach((lDevice) => {
        const desc = lDevice.getAttribute('desc');
        const lDeviceInst = `${lDevice.getAttribute('inst') ?? 'UNKNOWN_LDEVICE'}${desc ? ` (${desc})` : ''}`;
...
//# sourceMappingURL=dataAttributePicker.js.map
...
function getActionListShadow(editor) {
...
function clickDeleteButton(editor, index) {
...
async function clickMoreVertMenuItem(editor, moreVertIndex, menuItemIndex) {
    const shadow = getActionListShadow(editor);
    const otherActionList = shadow.querySelectorAll('oscd-list')[2];
    const moreVertButtons = otherActionList.querySelectorAll('oscd-list-item[id="more-vert-anchor"]');
    moreVertButtons[moreVertIndex].click();
    await timeout(100);
...
describe('DataSet element editor', () => {
    ...
    it('allows to change DataSets name attribute', async () => {
        ...
        expect(editEvent.args[0][0].detail.edit[0].attributes.name).to.equal('SomeDataSetName');
    });
...
//# sourceMappingURL=data-set-element-editor.spec.js.map
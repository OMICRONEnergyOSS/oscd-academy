...
import { dataSetDoc, otherDataSetDoc, dataSetDocWithDescs, } from './data-set-editor.testfiles.js';
...
describe('DataSet editor component', () => {
    describe('with missing SCL document', () => {
        ...
        it('looks like the latest snapshot', async () => {
            ...
            await timeout(200);
            await visualDiff(editor, `dataset/data-set-editor/#1 Missing SCL document`);
        });
    });
    describe('with SCL document loaded', () => {
        ...
        beforeEach(async () => {
            ...
            editor = await fixture(html `<data-set-editor .doc="${doc}"></data-set-editor>`);
            document.body.prepend(editor);
        });
        afterEach(async () => {
            editor.remove();
        ...
        describe('with unselected DataSet', () => {
            it('looks like the latest snapshot', async () => {
                ...
                await editor.updateComplete;
                await timeout(300);
                await visualDiff(editor, `dataset/data-set-editor/#2 Unselected DataSet 1900x1200`);
            });
            it('filtered looks like the latest snapshot', async () => {
                ...
                await editor.updateComplete;
                await timeout(400);
                await visualDiff(editor, `dataset/data-set-editor/#3 With filtered DataSets`);
            });
            it('on mobile looks like the latest snapshot', async () => {
                await setViewport({ width: 599, height: 1100 });
                await editor.updateComplete;
                await timeout(400);
                await visualDiff(editor, `dataset/data-set-editor/#4 Unselected DataSet 599x1100`);
            });
        });
        describe('with selected DataSet', () => {
            ...
            it('looks like the latest snapshot', async () => {
                await setViewport({ width: 1900, height: 1200 });
                await timeout(200);
                await visualDiff(editor, `dataset/data-set-editor/#5 Selected DataSet 1900x1200`);
            });
            it('on mobile looks like the latest snapshot', async () => {
                await setViewport({ width: 599, height: 1100 });
                await editor.updateComplete;
                await timeout(200);
                await visualDiff(editor, `dataset/data-set-editor/#6 Selected DataSet 599x1100`);
            });
            it('with active opened selection list looks like the latest snapshot', async () => {
                ...
                await editor.updateComplete;
                await timeout(200);
                await visualDiff(editor, `dataset/data-set-editor/#7 Selection List 599x1100`);
            });
            it('dynamically loaded new doc looks like the latest snapshot', async () => {
                await setViewport({ width: 599, height: 1100 });
                editor.selectDataSetButton.click();
                editor.doc = new DOMParser().parseFromString(otherDataSetDoc, 'application/xml');
                await editor.updateComplete;
                await timeout(200);
                await visualDiff(editor, `dataset/data-set-editor/#8 New Doc with selected DataSet 599x1100`);
            });
        });
    });
    describe('with SCL document containing DataSet descriptions', () => {
        let editor;
        beforeEach(async () => {
            const doc = new DOMParser().parseFromString(dataSetDocWithDescs, 'application/xml');
            editor = await fixture(html `<data-set-editor .doc="${doc}"></data-set-editor>`);
            document.body.prepend(editor);
        });
        afterEach(async () => {
        ...
        it('looks like the latest snapshot', async () => {
            ...
            await timeout(300);
            await editor.updateComplete;
            await visualDiff(editor, `dataset/data-set-editor/#9 Document shows descriptions in DataSet 1200x1600`);
        });
    });
});
